...
export default function FoundersManagement() {
  ...
  const fetchLeaders = async () => {
    try {
      ...
      if (res.ok) {
        ...
        // Filter only founders/directors
        const filtered = data.filter((l: any) =>
          /founder|director/i.test(l.role)
        );
        setLeaders(filtered);
      }
    } catch (e) {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("Are you sure you want to delete this founder/director?")) return;
    try {
  ...
  const handleAddLeader = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        fetchLeaders();
        setSuccessMessage(`Founder/Director successfully ${editingLeaderId ? "updated" : "added"}!`);
  ...
  return (
    <div className="max-w-7xl mx-auto pb-20">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-10 gap-4">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white tracking-tight mb-2">Founders &amp; Directors</h1>
          <p className="text-gray-500 dark:text-gray-400">
            Manage the core leadership displayed on the About page.
          </p>
        </div>
        <button
          onClick={() => {
        ...
        >
          <Plus className="w-5 h-5" /> Add Founder/Director
        </button>
      </div>
      ...
      <div className="bg-white dark:bg-gray-900 rounded-[2.5rem] shadow-sm border border-gray-100 dark:border-gray-800 overflow-hidden min-h-[400px]">
        {loading ? (
          ...
          <div className="w-full h-64 flex flex-col items-center justify-center text-gray-500">
            <p className="font-bold text-xl text-gray-900 dark:text-white mb-2">No Founders/Directors in Database</p>
            <p className="font-medium text-lg">Click 'Add Founder/Director' to create a profile.</p>
          </div>
        ) : (
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-gray-50 dark:bg-gray-800/30 border-b border-gray-100 dark:border-gray-800">
                  <th className="px-10 py-6 text-xs font-bold text-gray-400 uppercase tracking-widest leading-relaxed">Name</th>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-gray-900 w-full max-w-2xl rounded-[2.5rem] p-10 shadow-2xl relative border border-gray-100 dark:border-gray-800">
            ...
            <h2 className="text-3xl font-extrabold text-gray-900 dark:text-white mb-8 tracking-tight">
              {editingLeaderId ? "Edit Founder/Director" : "Add Founder/Director"}
            </h2>
            <form onSubmit={handleAddLeader} className="space-y-6">
              ...
              <div>
                ...
                <textarea rows={3} value={formData.description} onChange={e => setFormData({ ...formData, description: e.target.value })} className="w-full px-5 py-4 bg-gray-50 dark:bg-black border border-gray-200 dark:border-gray-800 rounded-xl focus:ring-2 focus:ring-primary outline-none dark:text-white font-medium resize-none leading-relaxed" placeholder="Short inspiring quote or bio..." />
              </div>
